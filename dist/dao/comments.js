...
class CommentsDao {
  ...
  static async createComments(v) {
    // 查询文章
    const article = await _article.Article.findByPk(v.get('body.article_id'));

    if (!article) {
      throw new global.errs.NotFound('没有找到相关文章');
    }

    const comments = new _comments.Comments();
  ...
  static async destroyComments(id) {
    const comments = await _comments.Comments.findOne({
      where: {
        id,
        deleted_at: null
      }
  ...
  static async getComments(id) {
    const comments = await _comments.Comments.scope('iv').findOne({
      where: {
        id,
        deleted_at: null
      }
  ...
  static async updateComments(id, v) {
    const comments = await _comments.Comments.findByPk(id);
  ...
  static async getCommentsList(page = 1) {
    const pageSize = 10;
    const comments = await _comments.Comments.scope('bh').findAndCountAll({
      limit: pageSize,
      // 每页10条
  ...
  static async getArticleComments(article_id, page = 1, desc = 'created_at') {
    const pageSize = 10;
    const comments = await _comments.Comments.findAndCountAll({
      where: {
        article_id,
        deleted_at: null
      },
...
exports.CommentsDao = CommentsDao;